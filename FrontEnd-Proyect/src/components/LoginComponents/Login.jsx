...
import { loginSession, current } from "../../Services/AuthAPI";
  ...
  const handletLogin = async (formdata) => {
    ...
    try {
      ...
      show_alert(
        ...
        "Error usuario y/0 contraseña incorrectos",
        "warning"
      );
      setisLoading(false);
      reset();
    ...
    <Fragment>
      <div className="divForm  p-4 px-5 shadow-lg">
        ...
        <form className="loginForm" onSubmit={handleSubmit(handletLogin)}>
          ...
          <div className="mb-3 w-100">
            <label htmlFor="passd" className="form-label">
              Contraseña
            </label>
            <input
              {...register("contrasena", {
            ...
            />
            {errors.contrasena && (
              <span className="requiredAlert">{errors.contrasena.message}</span>
            )}
          </div>
          <div className="mb-3 form-register text-center">
            <p>
              {" "}
              <NavLink to="/client/Register">Resgistrarme ahora </NavLink>
            </p>
          </div>

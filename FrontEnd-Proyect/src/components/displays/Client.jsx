...
export const Client = () => {
    ...
    const handleDeleteClient = async (id) => {
        try {
            const flag = await show_option('Eliminar', '¿Desea eliminar el cliente?', 'warning');
            console.log(flag);
            if (flag) {
    ...
    const confirmDeletePhone = async (phoneId) => {
        const flag = await show_option('Eliminar', '¿Estás seguro de eliminar este teléfono?', 'warning')
        if (flag) {
    ...
    const columns = [
        ...
        {
            field: 'telefono', headerName: 'Teléfonos', width: 150, renderCell: (params) => (
                <>
                    <FormControl variant="filled" sx={{ m: 0, minWidth: 150, padding: '2px',backgroundColor:'#fff' }}>
                        <Select
        ...
        },
        {
            field: 'Edit', headerName: 'Editar', width: 100, renderCell: (params) => (
                <>
                    <Actions {...{ params }} editFuntion={selectedClient} deleteFunction={handleDeleteClient} />
                </>
            )
        }
    ];
        ...
        <>
            <div className="container-fluid mt-3">
                ...
                <CustomModal
                    ...
                    content={<ClientForm control={control} handleSave={handleSubmit(onSubmit)} errors={errors} register={register} />}
                />
            </div>
        </>
    );

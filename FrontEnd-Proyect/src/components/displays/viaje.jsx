...
export const Viaje = () => {
    ...
    const handleDeleteViaje = async (id) => {
        try {
            const flag = await show_option('Eliminar', '¿Desea eliminar el viaje?', 'warning');
            if (flag) {
    ...
    const columns = [
        ...
        { field: 'vehiculo', headerName: 'Vehículo', width: 150, renderCell: (params) => params.row.vehiculo.tipoVehiculo },
        {
            field: 'empleados', headerName: 'Empleados', width: 300, renderCell: (params) => (
                <>
                    {params.row.empleados.map((empleado) => (
                        <div key={empleado.idEmpleado}>{`${empleado.nombre} ${empleado.apellido}`}</div>
                    ))}
                </>
            )
        },
        {
            field: 'Edit', headerName: 'Editar', width: 100, renderCell: (params) => (
                <Actions {...{ params }} editFuntion={showInfoViaje} deleteFunction={handleDeleteViaje} />
            )
        }
    ];

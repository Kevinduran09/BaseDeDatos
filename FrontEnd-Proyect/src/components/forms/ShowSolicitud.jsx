...
export const ShowSolicitud = () => {
    ...
    const populateForm = (data) => {
        ...
        setValue('numeros', data.cliente.telefonos.map((telefono) => `${telefono.tipoTelefono} : ${telefono.numeroTelefono}`).join(', '));
    ...
    const handletProcess = () => {
        if (solicitud.estado !== 'pendiente') {
            show_alert({
                title: 'No se puede proceder porque esta solicitud no está en pendiente',
                icon: 'warning'
            });
            return;
        }
    ...
    const cancelSolicitud = async () => {
        try {
            const flag = await show_option('¿Desea cancelar esta solicitud?', null, 'warning','Sí, cancelar');
                ...
                <Grid container spacing={3} sx={{ width: '100%' }}>
                    ...
                    <Grid item xs={12} sx={{ width: '100%', display: 'flex', justifyContent: 'end', gap: '10px' }}>
                        <Grid sx={{ display: 'flex', justifyContent: 'end', gap: '10px' }} >
                            <button onClick={() => navigate(-1)} type="button" className="btn btn-secondary" >Cerrar</button>
                            <button type="submit" className="btn btn-primary" data-bs-dismiss="modal" onClick={handletProcess}>Procesar</button>
                            <button type="button" className="btn btn-danger" data-bs-dismiss="modal" onClick={cancelSolicitud}>Cancelar Solicitud</button>
                        </Grid>
                    </Grid>
                </Grid>
            </form>

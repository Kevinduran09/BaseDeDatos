...
export const RecorridoForm = () => {
  ...
  useEffect(() => {
    const fetchEmpleados = async () => {
      try {
        ...
        let datos = response.data.filter(
          (empleado) =>
            empleado.puesto.cargo !== "Secretario" &&
            empleado.puesto.cargo !== "Contador" &&
            empleado.puesto.cargo !== "Administrador"
        );
        setEmpleados(datos);
      } catch (error) {
    ...
    const fetchData = async () => {
      try {
        ...
        setValue(
          "numeros",
          response.cliente.telefonos.map((t) => t.numeroTelefono).join(", ")
        );
  ...
  const handleProcesarRecorrido = async (formData) => {
    ...
    try {
      ...
      const res = await adminAPI.getViajeByFecha(
        dayjs(formData.hora).format("YYYY-MM-DD")
      );
      console.log(res);
      if (res.length > 0) {
        ...
        const availableViaje = existingViajes.find(
          (viaje) =>
            !viaje.recorridos.some(
              (recorrido) =>
                new Date(recorrido.solicitud.fecha).getTime() ===
                new Date(formData.hora).getTime()
            )
        );
    ...
    <div className="container pt-5">
      <form onSubmit={handleSubmit(handleProcesarRecorrido)}>
        <Grid container spacing={3}>
            ...
            <List>
              {viajeExistente
                ? selectedEmpleados.map((empleado, index) => (
                    <ListItem key={index}>
                      <ListItemText
                        primary={`${empleado.nombre} ${empleado.apellido} - ${empleado.puesto.cargo}`}
                      />
                    </ListItem>
                  ))
                : selectedEmpleados.map((empleado, index) => (
          ...
          <Grid item xs={6}>
            <FormControl fullWidth variant="outlined">
              <InputLabel id="vehiculo-select-label">
